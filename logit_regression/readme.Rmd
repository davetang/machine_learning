...
Logistic regression is a regression model where the dependent variable (DV) is categorical. Example from Wikipedia:
...
For this example, we’ll predict whether a car has high or low miles per gallon (MPG) based on gross horsepower; if `mpg` is higher than the median, we'll use a 1 to indicate that it is high and a 0 otherwise.
...
Use `glm()` to fit a logistic regression model that predicts `mpg_high` using `hp`.
...
The negative coefficient for `hp` (-0.05901) suggests that as horsepower (hp) increases, the likelihood of high MPG decreases.
